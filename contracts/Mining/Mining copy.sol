...
contract Mining {
    ...
    function participate(uint256 hashrate) external {
        ...
        miningParticipations[msg.sender] = MiningParticipation(
            msg.sender,
            hashrate
        );
    ...
    function chooseWinner() internal {
        ...
        uint256 highestNumber = hashrateCalculations[
            hashrateCalculations.length - 1
        ].endingNumber;
        ...
        for (uint256 i = 0; i < hashrateCalculations.length; i++) {
            if (
                randomNumber >= hashrateCalculations[i].startingNumber &&
                randomNumber <= hashrateCalculations[i].endingNumber
            ) {
        ...
        latestBlockNo++;
        blocks.push(
            BlockData(latestBlockNo, randomNumber, winner, block.timestamp)
        );
