/**
 *Submitted for verification at amoy.polygonscan.com on 2024-08-02
 */
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    {
        return string(abi.encodePacked(_uri, uint2str(_id)));
    }
...
contract BADGE is ERC1155, BasicAccessControl, Freezable {
    ...
    uint256 public constant ASCENDED = 5;

    uint256 public maxCommon = 10_00_000;
    uint256 public maxUnCommon = 10_00_000;
    uint256 public maxEpic = 10_00_000;
    uint256 public maxRare = 10_00_000;
    uint256 public constant MAX_ASCENDED = 111;

    uint256 public maxBadgeMint = 1;

    constructor() ERC1155("https://api.metadata.pentagon.games/badges/") {}
    ...
    function mint(
        ...
        require(
            ((balanceOf(_to, _id) + _amount) <= maxBadgeMint),
            "Cannot mint to the contract"
        );

        if (_id == COMMON) _mint(_to, _id, _amount, "WELCOME HERO BADGE");
        else if (_id == UNCOMMON) _mint(_to, _id, _amount, "NEW FRIEND BADGE");
        else if (_id == RARE) _mint(_to, _id, _amount, "DEGEN BADGE");
        else if (_id == EPIC) _mint(_to, _id, _amount, "ROYAL BADGE");
        else if (_id == ASCENDED) _mint(_to, _id, _amount, "BLACK BADGE");
    }
    ...
    function bulkMint(
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            ...
            require(
                ((balanceOf(_to[i], _id[i]) + _amount[i])) <= maxBadgeMint,
                "Max Badge minted"
            );

            if (_id[i] == COMMON)
                _mint(_to[i], _id[i], _amount[i], "WELCOME HERO BADGE");
            else if (_id[i] == UNCOMMON)
                _mint(_to[i], _id[i], _amount[i], "NEW FRIEND BADGE");
            else if (_id[i] == RARE)
                _mint(_to[i], _id[i], _amount[i], "DEGEN BADGE");
            else if (_id[i] == EPIC)
                _mint(_to[i], _id[i], _amount[i], "ROYAL BADGE");
            else if (_id[i] == ASCENDED)
                _mint(_to[i], _id[i], _amount[i], "BLACK BADGE");
        }
    }
    ...
    function mintBatch(
        ...
        require(!isContract(_to), "Cannot mint to the contract");
        uint256 isLengthValid = _ids.length ^ _amounts.length;
        require((isLengthValid == 0), "Length provided is invalid");

        bytes memory badgeData;

        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                (balanceOf(_to, _ids[i]) + _amounts[i]) <= maxBadgeMint,
                "Cannot mint to the contract"
            );

            if (_ids[i] == COMMON) {
                abi.encodePacked(badgeData, " WELCOME HERO BADGE ");
            } else if (_ids[i] == UNCOMMON) {
                abi.encodePacked(badgeData, " NEW FRIEND BADGE ");
            } else if (_ids[i] == RARE) {
                abi.encodePacked(badgeData, " DEGEN BADGE ");
            } else if (_ids[i] == EPIC) {
                abi.encodePacked(badgeData, " ROYAL BADGE ");
            } else if (_ids[i] == ASCENDED) {
                abi.encodePacked(badgeData, " BLACK BADGE ");
            }
        }
        _mintBatch(_to, _ids, _amounts, badgeData);
    }

    function setMaxBadgeMint(uint256 _maxBadgeMint) external onlyOwner {
        maxBadgeMint = _maxBadgeMint;
    }
    ...
    function burnBatch(uint256[] calldata ids, uint256[] calldata values)
        external
    {
