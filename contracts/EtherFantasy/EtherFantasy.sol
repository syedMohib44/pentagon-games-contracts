...
contract EtherFantasy is ERC721, BasicAccessControl, Freezable {
     ...
     */
    function applyPveBoost(uint256 tokenId) external onlyModerators {
        // OZ 5.0 uses _requireOwned
        require(!isPveBoosted[tokenId], "PVE boost already applied");

        Stats storage stats = characterStats[tokenId];

        // Use block.prevrandao instead of block.difficulty for newer chains
        uint256 randomSeed = uint256(
            keccak256(abi.encodePacked(block.prevrandao, tokenId))
        );
    ...
    function mint(
        ...
        // Use _ownerOf in OZ 5.0 instead of _exists
        require(
        ...
        // Assign pseudo-random base stats using _owner and _tokenId
        uint256 randomSeed = uint256(
            keccak256(abi.encodePacked(block.timestamp, _owner, _tokenId))
        );

        characterStats[_tokenId] = Stats({
            atk: uint16((randomSeed % 50) + 50),
            def: uint16(((randomSeed >> 8) % 50) + 50),
            hp: uint16(((randomSeed >> 16) % 100) + 100)
        });
