...
/**
 *Submitted for verification at polygonscan.com on 2023-10-06
 */
...
// File contracts/Blockchain_Superheroes_Distributor.sol
...
contract Blockchain_Superheroes_Distributor_PEN is BasicAccessControl {
    ...
    IERC20 public pentesterToken;

    address public blockchainSuperheroes;

    constructor(address _blockchainSuperheroes, IERC20 _pentesterToken) {
        blockchainSuperheroes = _blockchainSuperheroes;
        pentesterToken = IERC20(_pentesterToken);
    }

    uint256 public  mintingCount = 555555000000000;
    uint256 public mintingCap = 5555550000000002500;

    uint256 decimal = 18;

    // 10 PEN per NFT.
    uint256 public penPriceNFT = 5 * 10 ** decimal;
    uint256 public pentesterPriceNFT = 100 * 10 ** decimal;

    bool public _mintingPausedPEN = false;
    bool public _mintingPausedPENTESTER = false;
    ...
    function mint() external payable returns (bool) {
        ...
        require(!_mintingPausedPEN, "Minting paused");
        require(msg.value == penPriceNFT, "token price not met");
    ...
    function pentesterMint(uint256 _amount) external returns (bool) {
        IBlockchain_Superheroes blockchainSuperherosContract = IBlockchain_Superheroes(
                blockchainSuperheroes
            );
        ...
        require(!_mintingPausedPENTESTER, "Minting paused");
        require(mintingCount < mintingCap, "Max cap reached");
        require(
            _amount == pentesterPriceNFT,
            "Provided amount is invalid or wallet out of funds"
        );
        
        pentesterToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            _amount
        );
    ...
    function togglePausePEN() public onlyModerators {
        _mintingPausedPEN = !_mintingPausedPEN;
    }

    function togglePausePENTESTER() public onlyModerators {
        _mintingPausedPENTESTER = !_mintingPausedPENTESTER;
    }

    function updatePENPrice(uint256 _penPriceNFT) public onlyOwner {
        penPriceNFT = _penPriceNFT;
    }

    function updatePentesterPrice(uint256 _pentesterPriceNFT) public onlyOwner {
        pentesterPriceNFT = _pentesterPriceNFT;
    }
